...
(() => {
  ...
  /* Revelado basado en SCROLL (no depende de IntersectionObserver,
     que algunos navegadores/servidores no entregan de forma fiable).
     checkReveals() se llama al hacer scroll, al cargar y al redimensionar. */
  let revealEls = [];
  let chapterEls = [];

  const checkReveals = () => {
    if (!started) return;              // solo tras pulsar EMPEZAR
    ...
    for (const el of revealEls) {
      ...
      if (r.top < vh * 0.9 && r.bottom > 0) {
        ...
        const tw = el.matches(".typewriter") ? el : el.querySelector(".typewriter");
    ...
    for (const ch of chapterEls) {
      if (ch.dataset.unlocked) continue;
      const r = ch.getBoundingClientRect();
      if (r.top < vh * 0.55 && r.bottom > 0) {
        ch.dataset.unlocked = "1";
        const a = chapterAchievements[ch.id];
  ...
  const setupReveal = () => {
    revealEls = $$(".reveal");
    chapterEls = Object.keys(chapterAchievements)
  ...
  const PhotoSlots = (() => {
    ...
    const render = (slot, dataUrl, removable = true) => {
    ...
    // Carpeta y extensiones donde buscar imágenes fijas del proyecto.
    const FOLDER = "img/";
    const EXTS = ["jpg", "jpeg", "png", "webp", "JPG", "JPEG", "PNG", "WEBP"];

    // Intenta cargar img/<id>.<ext> probando varias extensiones.
    // Resuelve con la URL encontrada o null si ninguna existe.
    const tryFolder = (id) => new Promise((resolve) => {
      let i = 0;
      const probe = new Image();
      probe.onload = () => resolve(probe.src);
      probe.onerror = () => {
        i++;
        if (i < EXTS.length) probe.src = FOLDER + id + "." + EXTS[i];
        else resolve(null);
      };
      probe.src = FOLDER + id + "." + EXTS[0];
    });
    ...
    const init = () => {
      ...
      $$(".photo-slot").forEach(slot => {
        ...
        if (saved) {
          // El usuario subió una foto propia: tiene prioridad.
          render(slot, saved);
        } else {
          // Mientras tanto muestra el marcador y busca la imagen en img/.
          placeholder(slot);
          tryFolder(id).then(src => {
            // Solo si el usuario no subió nada entretanto.
            if (src && !localStorage.getItem(KEY(id))) render(slot, src, false);
          });
        }
  ...
  const beginExperience = () => {
    ...
    // Respaldo: garantiza que la portada quede oculta aunque la
    // transición no se complete (animaciones reducidas / pestaña en 2.º plano)
    setTimeout(() => { start.style.display = "none"; }, reduceMotion ? 0 : 950);

    window.scrollTo({ top: 0, behavior: "auto" });
    setTimeout(() => showAchievement("✨", "El comienzo · nuestra historia"), 900);
    updateLove();

    // Revela lo que ya esté en pantalla (prólogo) y deja listo el resto.
    // Varias llamadas escalonadas por si el layout aún se está asentando.
    [60, 250, 600, 1200].forEach(t => setTimeout(checkReveals, t));
  };
  ...
  const init = () => {
    ...
    addEventListener("scroll", () => {
      if (ticking) return;
      ticking = true;
      requestAnimationFrame(() => { updateLove(); checkReveals(); ticking = false; });
    }, { passive: true });
